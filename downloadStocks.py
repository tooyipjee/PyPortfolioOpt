...
portfolio.holding = pd.read_csv("C:/Users/Jason/Documents/GitHub/PyPortfolioOpt/symbols/"+ symbols +".csv")
...
for i, symbol in enumerate(portfolio.holding.Symbol):
    ...
    if (np.max(data.Close)/np.min(data.Close) < 20):
        ...
        portfolio.symbol_stats.append([symbol,mcap,delta,np.array(portfolio.holding["Holding"])[3]])
        print(symbol + " passed")
    else:
        print(symbol + " failed")
...
pf.optimise_portfolio(timeseries_data,portfolio)    
